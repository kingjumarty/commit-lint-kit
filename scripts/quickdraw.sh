...
TS="$(date +%Y%m%d-%H%M%S)"
TITLE="Quickdraw check — commit-lint-kit ($TS)"

echo "→ Opening issue..."
ISSUE_URL="$(gh issue create --repo "$REPO" \
  --title "$TITLE" \
  --body "Automated quickdraw issue created by scripts/quickdraw.sh at $TS. Verifying issue open/close flow for Commit Lint Kit.")"
